...
{
	m_RenderSpace = Render_Space::Render3D;
...
void CRenderManager::SetBloom(bool Enable)
{
	m_PostProcessCBuffer->SetBloom(Enable);
}
...
void CRenderManager::SetOutLine(bool Enable)
{
	m_PostProcessCBuffer->SetOutLine(Enable);
}

void CRenderManager::SetToonShader(bool Enable)
{
	m_PostProcessCBuffer->SetToonShader(Enable);
}

void CRenderManager::SetFXAA(bool Enable)
{
	m_PostProcessCBuffer->SetFXAA(Enable);
}
...
void CRenderManager::SetDefaultTarget()
{
	...
	CResourceManager::GetInst()->CreateTarget("PostProcess", RS.Width, RS.Height,
		DXGI_FORMAT_R8G8B8A8_UNORM);
	...
	m_PostProcessTarget = (CRenderTarget*)CResourceManager::GetInst()->FindTexture("PostProcess");
	m_PostProcessTarget->SetPos(200.f, 100.f, 0.f);
	m_PostProcessTarget->SetScale(100.f, 100.f, 1.f);
	m_PostProcessTarget->SetDebugRender(true);

	CResourceManager::GetInst()->CreateTarget("Shadow", (unsigned int)m_ShadowMapSize, (unsigned int)m_ShadowMapSize,
		DXGI_FORMAT_R32G32B32A32_FLOAT);

	m_ShadowTarget = (CRenderTarget*)CResourceManager::GetInst()->FindTexture("Shadow");
	m_ShadowTarget->SetPos(300.f, 0.f, 0.f);
	m_ShadowTarget->SetScale(100.f, 100.f, 1.f);
	...
	// Bloom----------
	CResourceManager::GetInst()->CreateTarget("Bloom", RS.Width, RS.Height,
		DXGI_FORMAT_R8G8B8A8_UNORM);

	m_BloomTarget = (CRenderTarget*)CResourceManager::GetInst()->FindTexture("Bloom");
	m_BloomTarget->SetPos(0.f, 0.f, 0.f);
	m_BloomTarget->SetScale(RS.Width / 2.f, RS.Height / 2.f, 1.f);
	m_BloomTarget->SetDebugRender(true);
	// ---------------
	...
	DepthDesc.Width = (UINT)m_ShadowMapSize;
	DepthDesc.Height = (UINT)m_ShadowMapSize;
...
void CRenderManager::Render(float DeltaTime)
{
	...
	m_PostProcessTarget->ClearTarget();
	...
	m_BloomTarget->ClearTarget();


	//if (CEngine::GetInst()->IsEditor())
	//{
	//	CScene* CurScene = CSceneManager::GetInst()->GetScene();
	//	CCamera* Camera = CurScene->GetCameraManager()->GetCurrentCamera();
	//	Matrix matView = Camera->GetViewMatrix();
	//	Matrix matProj = Camera->GetProjMatrix();
	//	ImGuizmo::DrawGrid(&matView[0][0], &matProj[0][0], identityMatrix, 200.f);
	//}
...
{
	// Sky 출력
	CLightComponent* GlobalLight = CSceneManager::GetInst()->GetScene()->GetLightManager()->GetGlobalLightComponent();

	//라이트값 dif등등 넘기기
	if (GlobalLight)
		GlobalLight->SetShader();
	...
	GlobalLight = CSceneManager::GetInst()->GetScene()->GetLightManager()->GetGlobalLightComponent();
...
void CRenderManager::RenderLightCombine(float DeltaTime)
{
	...
	m_vecGBuffer[4]->SetShader(14, TST_PIXEL);

	m_ShadowTarget->SetShader(19, TST_PIXEL);

	size_t	Size = m_vecLight.size();

	for (size_t i = 0; i < Size; ++i)
	{
		m_vecLight[i]->SetShader(16 + (int)i, TST_PIXEL);
	}
	...
	m_vecGBuffer[4]->ResetShader(14, TST_PIXEL);

	m_ShadowTarget->ResetShader(19, TST_PIXEL);

	for (size_t i = 0; i < Size; ++i)
	{
		m_vecLight[i]->ResetShader(16 + (int)i, TST_PIXEL);
	}
...
{
	m_DepthDisable->SetState();
	m_PostProcessTarget->SetTarget(nullptr);
	...
	m_DepthDisable->ResetState();
	m_PostProcessTarget->ResetTarget();
}

void CRenderManager::RenderBloomTarget(float DeltaTime)
{
	m_DepthDisable->SetState();

	m_BloomTarget->SetShader(0, TST_PIXEL);
...
void CRenderManager::RenderPostProcess(float DeltaTime)
{
	m_DepthDisable->SetState();
	m_FullScreenShader->SetShader();

	m_PostProcessTarget->SetShader(0, TST_PIXEL);
	m_vecGBuffer[0]->SetShader(50, TST_PIXEL);
	m_vecGBuffer[1]->SetShader(51, TST_PIXEL);
	m_vecGBuffer[2]->SetShader(52, TST_PIXEL);
	m_vecLight[0]->SetShader(53, TST_PIXEL);
	...
	m_PostProcessTarget->ResetShader(0, TST_PIXEL);
	m_vecGBuffer[0]->ResetShader(50, TST_PIXEL);
	m_vecGBuffer[1]->ResetShader(51, TST_PIXEL);
	m_vecGBuffer[2]->ResetShader(52, TST_PIXEL);
	m_vecLight[0]->ResetShader(53, TST_PIXEL);
...
void CRenderManager::AddPrimitiveComponent(CPrimitiveComponent* pPrimitive)
{
	if (pPrimitive->GetPrimitiveType() == PrimitiveComponent_Type::Primitive2D)
	{
		...
		// Bloom
		if (pPrimitive->GetBloomEnable())
		{
			if (m_RenderListBloomCapacity == 0)
			{
				m_RenderListBloomCapacity = 16;
				m_pRenderListBloom = new CPrimitiveComponent * [m_RenderListBloomCapacity];

				memset(m_pRenderListBloom, 0, sizeof(CPrimitiveComponent*) * m_RenderListBloomCapacity);
			}

			else if (m_RenderListBloomCapacity == m_RenderListBloomCount)
			{
				m_RenderListBloomCapacity *= 2;

				CPrimitiveComponent** pArray = new CPrimitiveComponent * [m_RenderListBloomCapacity];

				memset(pArray, 0, sizeof(CPrimitiveComponent*) * m_RenderListBloomCapacity);

				memcpy(pArray, m_pRenderListBloom, sizeof(CPrimitiveComponent*) * m_RenderListBloomCount);

				SAFE_DELETE_ARRAY(m_pRenderListBloom);

				m_pRenderListBloom = pArray;
			}

			m_pRenderListBloom[m_RenderListBloomCount] = pPrimitive;
			++m_RenderListBloomCount;
		}
