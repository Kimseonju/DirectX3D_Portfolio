...
std::vector<int> CSceneCollision::GetCollisionSection3D(const Vector3& Start, const Vector3& End)
{
	auto	iter = m_ColliderList.begin();
	auto	iterEnd = m_ColliderList.end();
	...
	CollisionSectionInfo* SectionInfo = m_Section3D;
...
//만약 다른거라면
			...
			{
				if (m_MouseCollisionWidget->GetZOrder() > (*iterUI)->GetZOrder())
					continue;
			}
