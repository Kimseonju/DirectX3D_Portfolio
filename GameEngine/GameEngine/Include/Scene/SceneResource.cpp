...
CSceneResource::~CSceneResource()
{
	...
	{
		auto    iter = m_mapSound2D.begin();
		auto    iterEnd = m_mapSound2D.end();

		for (; iter != iterEnd; ++iter)
		{
			iter->second->Release();
			CResourceManager::GetInst()->ReleaseSound2D(iter->first);
		}
	}
	{
		auto    iter = m_mapSound3D.begin();
		auto    iterEnd = m_mapSound3D.end();

		for (; iter != iterEnd; ++iter)
		{
			iter->second->Release();
			CResourceManager::GetInst()->ReleaseSound3D(iter->first);
		}
	}
	{
...
bool CSceneResource::Init()
{
	...
	pMesh = CResourceManager::GetInst()->FindMesh("TrailMesh");

	if (pMesh)
	{
		pMesh->m_pScene = m_pScene;
		pMesh->AddRef();
		m_mapMesh.insert(std::make_pair("TrailMesh", pMesh));
	}
	...
	const std::string& PathName)
{
	...
	if (!Sound)
	{
		...
		Sound = CResourceManager::GetInst()->FindSound2D(Name);

		Sound->SetScene(m_pScene);
		Sound->AddRef();
		m_mapSound2D.insert(std::make_pair(Name, Sound));
	}



	Sound = FindSound3D(Name);
	if (!Sound)
	{
		if (!CResourceManager::GetInst()->LoadSound(GroupName, Loop, Name, FileName, PathName))
			return false;

		Sound = CResourceManager::GetInst()->FindSound3D(Name);
		Sound->SetScene(m_pScene);
		Sound->AddRef();

		m_mapSound3D.insert(std::make_pair(Name, Sound));
	}
	return true;
}
...
bool CSceneResource::SoundPlay2D(const std::string& Name)
{
	return CResourceManager::GetInst()->SoundPlay2D(Name);
}

bool CSceneResource::SoundStop2D(const std::string& Name)
{
	return CResourceManager::GetInst()->SoundStop2D(Name);
}

bool CSceneResource::SoundPause2D(const std::string& Name)
{
	return CResourceManager::GetInst()->SoundPause2D(Name);
}

bool CSceneResource::SoundResume2D(const std::string& Name)
{
	return CResourceManager::GetInst()->SoundResume2D(Name);
}

bool CSceneResource::SoundPlay3D(const std::string& Name)
{
	return CResourceManager::GetInst()->SoundPlay3D(Name);
}

bool CSceneResource::SoundStop3D(const std::string& Name)
{
	return CResourceManager::GetInst()->SoundStop3D(Name);
}

bool CSceneResource::SoundPause3D(const std::string& Name)
{
	return CResourceManager::GetInst()->SoundPause3D(Name);
}

bool CSceneResource::SoundResume3D(const std::string& Name)
{
	return CResourceManager::GetInst()->SoundResume3D(Name);
}
...
CSound* CSceneResource::FindSound2D(const std::string& Name)
{
	auto    iter = m_mapSound2D.find(Name);
...
CSound* CSceneResource::FindSound3D(const std::string& Name)
{
	auto    iter = m_mapSound3D.find(Name);
...
bool CSceneResource::AddAnimationSequenceNotify(const std::string& SequenceName, const std::string& NotifyName, int Frame)
{
	CAnimationSequence* pSequence = FindAnimationSequence(SequenceName);

	if (!pSequence)
		return false;

	pSequence->AddNotify(NotifyName, (float)Frame);

	return true;
}

bool CSceneResource::AddAnimationSequenceSound2DNotify(const std::string& SequenceName,const std::string& SoundName,int Frame)
{
	CAnimationSequence* pSequence = FindAnimationSequence(SequenceName);
	CSound* Sound = FindSound2D(SoundName);

	if (!pSequence)
		return false;

	pSequence->AddSoundNotify(*Sound,Frame);

	return true;
}
bool CSceneResource::AddAnimationSequenceSound3DNotify(const std::string& SequenceName, const std::string& SoundName, int Frame)
{
	CAnimationSequence* pSequence = FindAnimationSequence(SequenceName);
	CSound* Sound = FindSound3D(SoundName);
