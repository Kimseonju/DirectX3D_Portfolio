...
void CTransform::SetQuaternion(const Vector3& vec)
{
    m_bQuaternion = true;
}

void CTransform::GetQuaternion()
{
}
...
{
	fwrite(&m_State, sizeof(Transform_State), 1, pFile);
...
{
	fread(&m_State, sizeof(Transform_State), 1, pFile);
...
Vector3 CTransform::GetLookAtYAxis(const Vector3& Pos)
{
	Vector3	Dir = Pos - m_WorldPos;
	Dir.y = 0.f;
	Dir.Normalize();
    ...
    return Vector3(Radian, Radian, Radian);
}

void CTransform::LookAtObject(CGameObject* Object)
{
    m_LookAtObject = Object;
}

void CTransform::LookAtObject()
{
    if (!m_LookAtObject)
        return;
	Vector3	Dir = m_LookAtObject->GetWorldPos() - m_WorldPos;
	Dir.y = 0.f;
	Dir.Normalize();
    ...
    if (m_LookAtObject->GetWorldPos().x > m_WorldPos.x)
        Angle = 360.f - Angle;
    SetWorldRotationY(Angle);

}

void CTransform::LookAtObjectActive(bool Enable)
{
    m_bLookAtObject = Enable;
}
...
void CTransform::PostUpdate(float DeltaTime)
{
    ...
    if (m_UpdateRot)
    {
        ...
        {
            m_matRot.Quaternion(m_QuaternionRotation);
        }
    }
...
void CTransform::SetTransform()
{
    ...
    m_pCBuffer->SetPosMatrix(m_matPos);

	m_pCBuffer->SetRotationMatrix(m_matRot);

    m_pCBuffer->SetScaleMatrix(m_matScale);
    ...
    // 최종 위치가 확정되고 난 후 현재 프레임의 최종 위치를 저장한다.
