...
Vector3 CSceneComponent::GetLookAtYAxis(const Vector3& Pos)
{
    return m_pTransform->GetLookAtYAxis(Pos);
}

void CSceneComponent::LookAtObject(CGameObject* Object)
{
	m_pTransform->LookAtObject(Object);
}

void CSceneComponent::LookAtObjectActive(bool Enable)
{
	m_pTransform->LookAtObjectActive(Enable);
}

void CSceneComponent::SetSocket(CBoneSocket* Socket)
{
    m_pTransform->SetSocket(Socket);
}
...
void CSceneComponent::Load(FILE* pFile)
{
	...
	fread(&m_SceneComponentType, sizeof(SceneComponent_Type), 1, pFile);
