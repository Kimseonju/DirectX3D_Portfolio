...
void CCollider::CheckPrevColliderSection()
{
	...
	for (; iter != iterEnd;)
	{
		...
		{
			CallCollisionCallback(Collision_State::End);
			if ((*iter)->IsActive() && (*iter)->IsEnable())
			{
				(*iter)->CallCollisionCallback(Collision_State::End);
...
bool CCollider::CollisionRay(HitResult& _HitResult, const Vector3& Start, const Vector3& Dir, float Length)
{
	return false;
}
...
void CCollider::Load(FILE* pFile)
{
	...
	fread(&m_ColliderShape, sizeof(Collider_Shape), 1, pFile);
	fread(&m_ColliderSpace, sizeof(Collider_Space), 1, pFile);
