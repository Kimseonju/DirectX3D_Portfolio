...
bool CInput::IsAnyInput()
{
	// 키보드, 마우스 상관없이 입력중인지 체크 
	bool result = false;
	unsigned int	Index;
	unsigned char	Key;

	for (size_t i = 0; i < m_vecAddIndexKey.size(); ++i)
	{
		Index = m_vecAddIndexKey[i];
		Key = m_vecKeyState[Index].Key;

		if (m_DirectInputKeyResult[Key] & 0x80)
		{
			result = true;
			break;
		}
