...
CAnimationSequence::CAnimationSequence(const CAnimationSequence& anim)
{
	*this = anim;
	...
	{
		m_vecSoundNotify.clear();

		auto	iter = anim.m_vecSoundNotify.begin();
		auto	iterEnd = anim.m_vecSoundNotify.end();

		for (; iter != iterEnd; ++iter)
		{
			AnimationSoundNotify* pNotify = new AnimationSoundNotify;

			pNotify->Name = (*iter)->Name;
			pNotify->Time = (*iter)->Time;
			pNotify->m_Sound = (*iter)->m_Sound;
			pNotify->pOwner = this;

			m_vecSoundNotify.push_back(pNotify);
		}
	}
}
...
CAnimationSequence::~CAnimationSequence()
{
	...
	{
		auto	iter = m_vecSoundNotify.begin();
		auto	iterEnd = m_vecSoundNotify.end();
...
bool CAnimationSequence::Save(const TCHAR* pFileName,const std::string& strPathName)
{
...
bool CAnimationSequence::SaveMultibyte(const char* pFileName,const std::string& strPathName)
{
...
bool CAnimationSequence::Load(const TCHAR* pFileName,const std::string& strPathName)
{
	char	strFileName[MAX_PATH] = {};
...
bool CAnimationSequence::LoadMultibyte(const char* pFileName,const std::string& strPathName)
{
	const PathInfo* pPath = CPathManager::GetInst()->FindPath(strPathName);
...
bool CAnimationSequence::LoadFullPathMultibyte(const char* pFullPath)
{
	...
	m_KeyFrameBuffer->Init("KeyFrameBuffer", sizeof(AnimationFrameTrans),
		(unsigned int)vecFrameTrans.size(), 13, true, CBT_COMPUTE);

	m_KeyFrameBuffer->UpdateBuffer(&vecFrameTrans[0],
		sizeof(AnimationFrameTrans) * (int)vecFrameTrans.size());
...
void CAnimationSequence::SetEndFrame(int Frame)
{
	m_FrameLength = Frame;
}
...
void CAnimationSequence::SetPlayFrame(int Frame)
{
	m_PlayTime = (float)m_FrameLength / (float)Frame;
...
bool CAnimationSequence::CreateSequence(bool bLoop,_tagFbxAnimationClip* pClip)
{
...
bool CAnimationSequence::CreateSequence(const std::string& strName, bool bLoop, int iStartFrame,int iEndFrame, float fPlayTime,const std::vector<BoneKeyFrame*>& vecFrame)
{
	// 인자로 들어온 애니메이션 옵션정보를 설정한다.
	m_Loop = bLoop;
	SetName(strName);
	...
	m_KeyFrameBuffer->Init("KeyFrameBuffer", sizeof(AnimationFrameTrans),(unsigned int)vecFrameTrans.size(), 13, true, CBT_COMPUTE);
...
bool CAnimationSequence::CreateSequence(bool bLoop, const std::string& strName,const TCHAR* pFileName, const std::string& strPathName)
{
	SetName(strName);
...
void CAnimationSequence::AddSoundNotify(CSound& Sound, float Time)
{
	AnimationSoundNotify* pNotify = new AnimationSoundNotify;
	std::string Name = {};
	Name = std::to_string(m_vecSoundNotify.size());
	pNotify->Name = Name;
	pNotify->Time = Time;
	pNotify->pOwner = this;
	pNotify->m_Sound= &Sound;
