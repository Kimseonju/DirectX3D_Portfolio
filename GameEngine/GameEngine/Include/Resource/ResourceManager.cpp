...
bool CResourceManager::Init()
{
	...
	if (FMODResult != FMOD_OK)
		return false;
	//FMODResult = m_System->init(128, FMOD_INIT_NORMAL| FMOD_INIT_3D_RIGHTHANDED, nullptr);
	//
	//if (FMODResult != FMOD_OK)
	//	return false;
	// MasterGroup을 얻어온다.
	...
	if (!CreateMesh(Mesh_Type::Sprite, "TextureRectCenter", TextureRectCenter, 4, sizeof(VertexUV), D3D11_USAGE_DEFAULT,
		D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST, IdxRect, 6, sizeof(int), D3D11_USAGE_DEFAULT,
		DXGI_FORMAT_R32_UINT))
		return false;
	Vector3	Box2DPos[5] =
	...
	VertexUV vVertextex[20] = {};

	if (!CreateMesh(Mesh_Type::StaticMesh, "TrailMesh", BillboardMesh, 4, sizeof(VertexUV),
		D3D11_USAGE_DEFAULT,
		D3D11_PRIMITIVE_TOPOLOGY_POINTLIST, IdxRect, 6, sizeof(int), D3D11_USAGE_DEFAULT,
		DXGI_FORMAT_R32_UINT))
		return false;

	//if (!CreateMesh(Mesh_Type::Sprite, "TrailMesh", vVertextex,
	//	20, sizeof(VertexUV), D3D11_USAGE_IMMUTABLE, D3D11_PRIMITIVE_TOPOLOGY_POINTLIST))
	//	return false;
	...
	if (!LoadTexture("DefaultDistortionNoise", TEXT("noise_cloud.TGA")))
		return false;
	...
	CreateMesh(Mesh_Type::StaticMesh, "SpherePos", &vecSpherePos[0], SphereSize, sizeof(Vector3),
		D3D11_USAGE_DEFAULT, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST, &vecSphereIndex[0], (int)vecSphereIndex.size(),
	...
	SkyMtrl->AddTexture("Sky", TEXT("Sky/StarSky.dds"));
	SkyMtrl->SetTextureRegister("Sky", 20);
	SkyMtrl->SetTextureLink("Sky", Texture_Link::CustomTexture);
...
void CResourceManager::Update()
{
	...
	FMOD_VECTOR FForward = Camera->GetAxis(AXIS_Z).FMODConvert();
	FMOD_VECTOR FUp = Camera->GetAxis(AXIS_Y).FMODConvert();

	m_System->set3DListenerAttributes(0, &FPos,  NULL, &FForward, &FUp);
	m_System->update();
}
	...
	const std::string& Name, const char* FileName, const std::string& PathName)
{
	...
	}
	{
		...
		if (!Sound)
		{
			...
			if (!Sound->LoadSound(m_System, Group, Loop, Name, FileName, PathName, FMOD_3D))
			{
				SAFE_RELEASE(Sound);
				return false;
			}
...
bool CResourceManager::SoundPlay2D(const std::string& Name)
{
	CSound* Sound = FindSound2D(Name);
...
bool CResourceManager::SoundPlay3D(const std::string& Name)
{
	CSound* Sound = FindSound3D(Name);
...
bool CResourceManager::SoundStop2D(const std::string& Name)
{
	CSound* Sound = FindSound2D(Name);
...
bool CResourceManager::SoundStop3D(const std::string& Name)
{
	CSound* Sound = FindSound3D(Name);
...
bool CResourceManager::SoundPause2D(const std::string& Name)
{
	CSound* Sound = FindSound2D(Name);
...
bool CResourceManager::SoundPause3D(const std::string& Name)
{
	CSound* Sound = FindSound3D(Name);
...
bool CResourceManager::SoundResume2D(const std::string& Name)
{
	CSound* Sound = FindSound2D(Name);
...
bool CResourceManager::SoundResume3D(const std::string& Name)
{
	CSound* Sound = FindSound3D(Name);
...
void CResourceManager::ReleaseSound2D(const std::string& Name)
{
	...
	{
		iter->second->Release();
		m_mapSound2D.erase(iter);
	}
}
void CResourceManager::ReleaseSound3D(const std::string& Name)
{
...
CSound* CResourceManager::FindSound2D(const std::string& Name)
{
	auto    iter = m_mapSound2D.find(Name);
...
CSound* CResourceManager::FindSound3D(const std::string& Name)
{
	auto    iter = m_mapSound3D.find(Name);
