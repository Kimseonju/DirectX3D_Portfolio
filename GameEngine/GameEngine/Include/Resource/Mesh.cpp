...
void CMesh::AllLightMaterialReset()
{
    size_t Size = m_vecMaterialSlot.size();

    for (size_t i = 0; i < Size; ++i)
	{
		m_vecMaterialSlot[i]->SetBaseColor(1.f, 1.f, 1.f, 1.f);
		m_vecMaterialSlot[i]->SetEmissiveColor(0.3f, 0.3f, 0.3f, 1.f);
		m_vecMaterialSlot[i]->SetSpecularColor(0.f, 0.f, 0.f, 1.f);
        m_vecMaterialSlot[i]->SetBumpEnable(false);
    }
}

void CMesh::AllShadowEnable(bool bEnable)
{
	size_t Size = m_vecMaterialSlot.size();

	for (size_t i = 0; i < Size; ++i)
	{
		m_vecMaterialSlot[i]->SetShadowEnable(bEnable);
	}
}
