...
void CMaterial::SetBumpEnable(bool Enable)
{
	m_Bump = Enable;

	m_pCBuffer->BumpEnable(Enable);
}
...
void CMaterial::SetFlowEnable(bool bFlow)
{
	m_bFlow = bFlow;
	m_pCBuffer->FlowEnable(m_bFlow);
}

void CMaterial::SetFlowDirection(int bFlowDirection)
{
	// 0 : x
	// 1 : y
	// 2 : xy
	m_FlowDirection = bFlowDirection;
	m_pCBuffer->SetFlowDirection(m_FlowDirection);
}

void CMaterial::SetFlowSpeedX(float FlowSpeedX)
{
	m_FlowSpeedX = FlowSpeedX;
	m_pCBuffer->SetFlowSpeedX(m_FlowSpeedX);
}

void CMaterial::SetFlowSpeedY(float FlowSpeedY)
{
	m_FlowSpeedY = FlowSpeedY;
	m_pCBuffer->SetFlowSpeedY(m_FlowSpeedY);
}

void CMaterial::SetFlowSpeed(float FlowSpeedX, float FlowSpeedY)
{
	m_FlowSpeedX = FlowSpeedX;
	m_FlowSpeedY = FlowSpeedY;
	m_pCBuffer->SetFlowSpeed(m_FlowSpeedX, m_FlowSpeedY);
}
...
bool CMaterial::SetTexture(int Index, const std::string& Name, const TCHAR* FileName, const std::string& PathName)
{
...
bool CMaterial::SetTextureFullPath(int Index, const std::string& Name, const TCHAR* FullPath)
{
...
void CMaterial::BloomEnable(bool Enable)
{
	m_BloomEnable = Enable;
	m_pCBuffer->BloomEnable(Enable);
}
...
void CMaterial::SetShadowEnable(bool Enable)
{
	m_pCBuffer->ShadowEnable(Enable);
}
...
void CMaterial::SetMaterial()
{
	...
	for (size_t i = 0; i < Size; ++i)
	{
		m_vecTexture[i]->pTexture->SetShader(m_vecTexture[i]->Register, m_vecTexture[i]->ShaderType);
	}
...
void CMaterial::ResetMaterial()
{
	...
	for (size_t i = 0; i < Size; ++i)
	{
		m_vecTexture[i]->pTexture->ResetShader(m_vecTexture[i]->Register,m_vecTexture[i]->ShaderType);
	}
