...
bool CStructuredBuffer::InitRead(const std::string& Name, unsigned int Size, unsigned int Count, int Register, int ConstantBufferType)
{
	SAFE_RELEASE(m_SRV);
	SAFE_RELEASE(m_UAV);
	SAFE_RELEASE(m_Buffer);
...
void CStructuredBuffer::UpdateBuffer(void* Data, int Size)
{
	...
	ID3D11DeviceContext* Content = CONTEXT;
	Content->Map(m_Buffer, 0, D3D11_MAP::D3D11_MAP_WRITE_DISCARD, 0, &Map);
