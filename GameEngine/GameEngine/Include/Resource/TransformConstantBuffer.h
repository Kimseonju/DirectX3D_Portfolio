...
public:
    void SetPosMatrix(const Matrix& matPos)
    {
        m_BufferData.matPos = matPos;
    }
	void SetRotationMatrix(const Matrix& matRot)
	{
		m_BufferData.matRot = matRot;
	}
    void SetScaleMatrix(const Matrix& Scale)
    {
        m_BufferData.matScale = Scale;
    }
