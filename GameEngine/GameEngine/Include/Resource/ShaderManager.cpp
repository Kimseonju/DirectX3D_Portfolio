...
bool CShaderManager::Init()
{
	...
	if (!CreateShader<CTrailShader>("TrailShader"))
		return false;

	if (!CreateShader<CPoseableAnimationShader>("PoseableAnimationShader"))
		return false;

	if (!CreateShader<CNotLightShader>("NotLightShader"))
		return false;
    ...
    CreateConstantBuffer("BloomCBuffer", sizeof(BloomCBuffer), 8, CBT_PIXEL);
	...
	CreateConstantBuffer("ShadowCBuffer", sizeof(ShadowCBuffer), 12, CBT_VERTEX | CBT_PIXEL);
	CreateConstantBuffer("TrailCBuffer", sizeof(TrailCBuffer), 13, CBT_VERTEX | CBT_COMPUTE | CBT_PIXEL | CBT_GEOMETRY);
