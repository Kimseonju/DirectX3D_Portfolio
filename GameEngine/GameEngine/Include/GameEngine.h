...
struct Vertex3D
{
	...
	Vertex3D(const Vector3& _Pos, const Vector3& Normal, const Vector2& _UV, const Vector3& Tangent)
		: Pos(_Pos), UV(_UV), Normal(Normal), Tangent(Tangent)
	{
	}
...
struct TrailCBuffer
{
	Vector3	TrailCurrentPos;
	float	TrailScale;
	Vector4	TrailVelocity[30];
	Vector4	TrailVelocity2[30];
};