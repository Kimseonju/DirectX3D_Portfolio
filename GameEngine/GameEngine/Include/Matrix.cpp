...
void Matrix::Quaternion(const Vector4& _v)
{
	m = XMMatrixRotationQuaternion(_v.Convert());
}
...
Matrix Matrix::GetXMMatrixDecompose(const Matrix& _m)
{
	XMVECTOR outScale;
	XMVECTOR outRotQuat;
	XMVECTOR outTrans;
	XMMatrixDecompose(&outScale, &outRotQuat, &outTrans, _m.m);

	v[0];
	float posx = XMVectorGetX(outTrans);
	float posy = XMVectorGetY(outTrans);
	float posz = XMVectorGetZ(outTrans);

	float Rotx = XMVectorGetX(outRotQuat);
	float Roty = XMVectorGetY(outRotQuat);
	float Rotz = XMVectorGetZ(outRotQuat);

	float Scalex = XMVectorGetX(outScale);
	float Scaley = XMVectorGetY(outScale);
	float Scalez = XMVectorGetZ(outScale);
	v[0].x = posx;
	v[0].y = posy;
	v[0].z = posz;

	v[1].x = XMConvertToDegrees(Rotx);
	v[1].y = XMConvertToDegrees(Roty);
	v[1].z = XMConvertToDegrees(Rotz);
