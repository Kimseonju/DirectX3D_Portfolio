...
RWStructuredBuffer<matrix>		g_BoneSocketRootMatrix	: register(u2);
...
void AnimationUpdateCS(int3 ThreadID : SV_DispatchThreadID)
{
	...
	uint RootFrameIndex = g_AnimRootBoneIndex * g_AnimFrameCount + g_AnimCurrentFrame;
	uint RootFrameNextIndex = g_AnimRootBoneIndex * g_AnimFrameCount + g_AnimNextFrame;
	...
	float4	RootPos = lerp(g_FrameTransArray[RootFrameIndex].Translate, g_FrameTransArray[RootFrameNextIndex].Translate, g_AnimRatio);
	//본인덱스라면 따로저장

	if (g_AnimChangeAnimation == 1)
	{
		uint ChangeFrameIndex = ThreadID.x * g_AnimChangeFrameCount;
		uint ChangeRootFrameNextIndex = g_AnimRootBoneIndex * g_AnimChangeFrameCount;
		...
		RootPos = lerp(RootPos, g_ChangeFrameTransArray[ChangeRootFrameNextIndex].Translate, g_AnimChangeRatio);
	
	}

	//일단 모든 캐릭터는 멈춰있다.
	if (ThreadID.x == g_AnimRootBoneIndex)
	{
		//맨마지막슬롯에 이동크기를 저장
		MatrixAffineTransformation(Scale, ZeroRot, Rot, Pos, matBone);
		g_BoneSocketRootMatrix[0] = transpose(matBone);
		matBone = (matrix)0.f;
	}
	Pos.y -= RootPos.y;
	Pos.x -= RootPos.x;
